 CREATE TABLE `simplecomic`.`comics` (
...
INDEX `bydate` ( `pub_date` ) ,
 ...
 CREATE TABLE  `simplecomic`.`chapters` (
...
CREATE TABLE  `simplecomic`.`chapters_text` (
 `chapterid` INT NOT NULL ,
 `description` MEDIUMTEXT NOT NULL ,
 PRIMARY KEY (  `chapterid` )
 ) ENGINE = MYISAM CHARACTER SET utf8 COLLATE utf8_general_ci;

CREATE TABLE `simplecomic`.`rants` (
`rantid` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
`title` VARCHAR( 128 ) NOT NULL ,
`pub_date` INT NOT NULL
) ENGINE = MYISAM ;

 CREATE TABLE  `simplecomic`.`rants_text` (
 `rantid` INT NOT NULL ,
